...
void CElementManager::LoadModule(CModeler1View * pView)
{
	...
	String^ asmName = gcnew String((LPTSTR)(LPCTSTR) fileName);
	List<ExtractorData^>^ data = Extractor::ExtractTypes(asmName);
	if (data == nullptr)
		return;
	...
	{
		std::shared_ptr<CElement> pNewElement = CFactory::CreateElementOfType(ElementType::type_shapes_import,
			ShapeType::import_class);
		CElement * pElement = pNewElement.get();
		CImportElement * pImportElement = (CImportElement *)pElement;

		String^ s1 = Convert::ToString(c);
		String^ s2 = Convert::ToString(data[c]->Name);
		String^ s3 = Convert::ToString(data[c]->FullName);

		pNewElement->m_text = (CStringW)s2;

		std::shared_ptr<CTypeData> ptd = make_shared<CTypeData>();
		ptd->m_Name = (CStringW)s2;
		ptd->m_FullName = (CStringW)s3;

		int countMethods = data[c]->Methods->Count;
		for (int m = 0; m < countMethods; ++m)
		{
			bool isPublic = data[c]->Methods[m]->IsPublic;
			String^ nameMethod = data[c]->Methods[m]->Name;

			std::shared_ptr<CMethodData> pmd = make_shared<CMethodData>();
			pmd->m_IsPublic = isPublic;
			pmd->m_Name = (CStringW)nameMethod;

			ptd->m_Methods.push_back(pmd);
		}

		int countFields = data[c]->Fields->Count;
		for (int f = 0; f < countFields; ++f)
		{
			bool isPublic = data[c]->Fields[f]->IsPublic;
			String^ typeField = data[c]->Fields[f]->Type;
			String^ nameField = data[c]->Fields[f]->Name;

			std::shared_ptr<CFieldData> pfd = make_shared<CFieldData>();
			pfd->m_IsPublic = isPublic;
			pfd->m_Type = (CStringW)typeField;
			pfd->m_Name = (CStringW)nameField;

			ptd->m_Fields.push_back(pfd);
		}

		int countProperties = data[c]->Properties->Count;
		for (int p = 0; p < countProperties; ++p)
		{
			String^ typeProperty = data[c]->Properties[p]->Type;
			String^ nameProperty = data[c]->Properties[p]->Name;

			std::shared_ptr<CPropertyData> ppd = make_shared<CPropertyData>();
			ppd->m_Type = (CStringW)typeProperty;
			ppd->m_Name = (CStringW)nameProperty;

			ptd->m_Properties.push_back(ppd);
		}

		pImportElement->m_TypeData = ptd;

		CalcAutoPointRect(c, pNewElement);
		pNewElement->m_pManager = this;
		pNewElement->m_pView = pView;
