...
void CElement::SetGuid()
{
	...
	this->m_objectId = T2W((LPCTSTR)objectId);
#endif
}
...
void CElement::Serialize(CArchive& ar)
{
	...
	if (ar.IsStoring())
	{
		CString name = W2T(m_name.c_str());
		ar << name;
		int type = m_type;
		ar << type;
		int shapeType = m_shapeType;
		ar << shapeType;
		CString objectId = W2T(m_objectId.c_str());
		ar << objectId;
		CString caption = W2T(m_caption.c_str());
		ar << caption;
		CString text = W2T(m_text.c_str());
		ar << text;
		ar << m_rect;
		ar << m_point;
		ar << m_last;
		...
		ar << m_bColorLine;
		ar << m_lineWidth;
		CString image = W2T(m_image.c_str());
		ar << image;
	}
	else
	{
		...
		CString caption;
		ar >> caption;
		this->m_caption = T2W((LPCTSTR)caption);
		CString text;
		ar >> text;
		this->m_text = T2W((LPCTSTR)text);
		ar >> m_rect;
		ar >> m_point;
		ar >> m_last;
		ar >> m_colorFill;
		...
		CString image;
		ar >> image;
		this->m_image = T2W((LPCTSTR)image);
	}
}
